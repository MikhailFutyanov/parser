...
class MoviesSpider(scrapy.Spider):
    ...
    def parse_movie(self, response):
        ...
        for row in rows:
            ...
            raw_values = row.xpath('.//td//text()').getall() #фильтрация на css стили
            cleaned_values = [v.strip() for v in raw_values if v.strip() and "{" not in v and "}" not in v]
            ...
            elif key in ["Год", "Год выпуска", "Премьера", "Дата выхода"]:
                movie["Год"] = value
